...
def _sample_df_new() -> pd.DataFrame:
    df_high_card = pd.DataFrame({
        "id": range(100),
        "high_card_col": [f"value_{i}" for i in range(100)],
    })

    new_row = pd.DataFrame({"id": [1], "high_card_col": ["value_1"]})
    df_high_card = pd.concat([df_high_card, new_row], ignore_index=True)
    return df_high_card
