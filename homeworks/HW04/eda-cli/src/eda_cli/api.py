...
class QualityFlagsResponse(BaseModel):
    """Полный набор флагов качества данных из CSV-файла."""
    flags: dict[str, Any] = Field(
        ...,
        description="Полный набор флагов качества данных, включая новые эвристики из HW03",
    )
...
@app.post(
    ...
    tags=["quality"],
    summary="Полный набор флагов качества из CSV-файла с использованием новых эвристик HW03",
)
async def quality_flags_from_csv(file: UploadFile = File(...)) -> QualityFlagsResponse:
    """
    Эндпоинт, который принимает CSV-файл, запускает EDA-ядро
    и возвращает полный набор флагов качества данных, включая новые эвристики из HW03.
    """
